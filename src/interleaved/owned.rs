...
impl<S: Sample + Default> Interleaved<S> {
...
impl<S: Sample> Contiguous<S> for Interleaved<S> {
    #[nonblocking]
    fn raw_data(&self) -> &[S] {
        self.raw_data()
    }
...
impl<S: Sample> ContiguousMut<S> for Interleaved<S> {
    #[nonblocking]
...
mod tests {
    ...
    fn test_member_functions() {
        let mut block = Interleaved::<f32>::new(4, 3);
